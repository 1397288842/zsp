...
namespace evpp
{
    ...
    void TCPConn::Close()
    {
        LogDebug("fd:{} status:{} addr:{}", fd_, StatusToString(), AddrToString());
    ...
    void TCPConn::HandleWrite()
    {
        ...
        else
        {
            ...
            if (EVUTIL_ERR_RW_RETRIABLE(serrno))
            {
                LogError("TCPConn::HandleWrite errno:{} {}", serrno, strerror(serrno));
            }
            else
            {
                HandleError();
    ...
    {
        assert(loop_->IsInLoopThread());
        LogInfo("DelayClose addr:{} fd:{} status_:{}", AddrToString(), fd_, StatusToString());
        status_ = kDisconnecting;
    ...
    {
        LogInfo("HandleClose addr:{} fd:{} status_:{}", AddrToString(), fd_, StatusToString());
        ...
        {
            LogInfo("Cancel the delay closing timer.");
        ...
        LogInfo("HandleClose addr:{} fd:{} status_:{} use_count:{}", AddrToString(), fd_, StatusToString(), conn.use_count());
        status_ = kDisconnected;
    }

    void TCPConn::HandleError()
    {
        LogDebug("fd:{} status:{}", fd_, StatusToString());
        status_ = kDisconnecting;
        HandleClose();
    }
