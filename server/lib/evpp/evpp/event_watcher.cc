...
bool EventWatcher::Watch(Duration timeout)
{
    ...
    if (attached_)
    {
        ...
        {
            LogError("event_del failed. fd:{} ", this->event_->ev_fd);
    ...
    {
        LogError("event_add failed. fd:{} ", this->event_->ev_fd);
        return false;
    }
    attached_ = true;
    return true;
...
bool PipeEventWatcher::DoInit()
{
    ...
    {
        int err = errno;
        LogError("create socketpair ERROR errno={} {}", err, strerror(err));
        goto failed;
    }
...
{
    LogTrace("PipeEventWatcher::HandlerFn fd:{}", fd);
