...
TCPServer::TCPServer(EventLoop* loop,
    ...
    , next_conn_id_(0)
{
    LogInfo("create TCPServer name:{} addr:{}  thread_num:{}", name, laddr, thread_num);
...
void TCPServer::StopInLoop(DoneCallback on_stopped_cb)
{
    ...
    else
    {
        ...
        for (auto& c : connections_)
        {
            ...
            {
                LogInfo("close connection id:{} fd:{}", c.second->id(), c.second->fd());
                c.second->Close();
            }
            else
            {
                LogInfo("Do not need to call Close for this TCPConn it may be doing disconnecting. fd:{} status:{}", c.second->fd(), StatusToString());
            }
        }
    ...
    LogInfo("TCPServer exited, status:{}", StatusToString());
}
...
{
    LogInfo("TCPServer::HandleNewConn fd:{}", sockfd);
    assert(loop_->IsInLoopThread());
    if (IsStopping())
    {
        //服务关闭中，新来的连接直接关闭掉
        LogWarn("The server is at stopping status. Discard this socket fd:{} remote_addr:{}", sockfd, remote_addr);
...
{
    LogInfo("TCPServer::RemoveConnection fd:{} connections_.size():{}", conn->fd(), connections_.size());
    auto f = [this, conn]()
    {
        // Remove the connection in the listening EventLoop
        LogInfo("TCPServer::RemoveConnection in loop fd:{} connections_.size():{}", conn->fd(), connections_.size());
        ...
        if (IsStopping() && this->connections_.empty())
        {
            ...
            LogInfo("remove all connections stop thread pool");
