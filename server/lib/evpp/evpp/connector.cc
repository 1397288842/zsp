...
{
    LogTrace("{} status:{}", remote_addr_, StatusToString());
    if (status_ == kDisconnected)
    {
        ...
        LogInfo("fd:{} remote_addr:{} receive write event when socket is closed", chan_->fd(), remote_addr_);
        return;
    }
    ...
    {
        err = errno;
        LogError("getsockopt failed err:{} {}", err, strerror(err));
    }
...
{
    LogTrace("HandleError remote_addr_:{} status:{} ", remote_addr_, StatusToString());
    assert(loop_->IsInLoopThread());
    int serrno = errno;
    ...
    LogInfo("connection Disconnect by remote peer fd:{} remote_addr_:{}", fd_, remote_addr_);
    ...
    if (owner_tcp_client_->auto_reconnect())
    {
        ...
        if (fd_ > 0)
        {
            LogTrace("Connector::HandleError close fd:{}", fd_);
            assert(own_fd_);
            EVUTIL_CLOSESOCKET(fd_);
            fd_ = INVALID_SOCKET;
        }

        LogInfo(" auto reconnect will in {}s thread:{}", owner_tcp_client_->reconnect_interval().Seconds(), std::this_thread::get_id());
...
{
    LogWarn("Connector::OnConnectTimeout status: fd:{}", StatusToString(), fd_);
...
{
    LogTrace("OnDNSResolved addrs.size:{}", addrs.size());
    if (addrs.empty())
    {
        LogError(" DNS Resolve failed. host:{}", dns_resolver_->host());
        HandleError();
        return;
    }
