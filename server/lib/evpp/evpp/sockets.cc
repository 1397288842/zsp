...
evpp_socket_t CreateNonblockingSocket()
{
    ...
    {
        serrno = errno;
        LogError("socket error:{} ", strerror(serrno));
    ...
    {
        serrno = errno;
        LogFatal("fcntl(F_SETFD):{}", strerror(serrno));
        goto out;
    }
#endif
...
evpp_socket_t CreateUDPServer(int port)
{
    ...
    {
        int serrno = errno;
        LogError("socket bind error:{} {}", serrno, strerror(serrno));
        return INVALID_SOCKET;
    }
...
bool ParseFromIPPort(const char* address, struct sockaddr_storage& ss)
{
    ...
    if (rc == 0)
    {
        LogInfo("ParseFromIPPort evutil_inet_pton (AF_INET '{}', ...) rc=0. {} is not a valid IP address. Maybe it is a hostname.", host, host);
        return false;
    }
    else if (rc < 0)
    {
        ...
        if (serrno == 0)
        {
            LogInfo("[{}] is not a IP address. Maybe it is a hostname.", host);
        }
        else
        {
            LogInfo("ParseFromIPPort evutil_inet_pton (AF_INET, '{}', ...) failed : ", host, strerror(serrno));
        }
        return false;
    }
...
bool SplitHostPort(const char* address, std::string& host, int& port)
{
    ...
    if (index == std::string::npos)
    {
        LogError("Address specified error <{}>. Cannot find ':'", address);
        return false;
    }
    ...
    if (host[0] == '[')
    {
        if (*host.rbegin() != ']')
        {
            LogError("Address specified error <{}>. '[' ']' is not pair.", address);
            return false;
        }
...
struct sockaddr_storage GetLocalAddr(evpp_socket_t sockfd)
{
    ...
    {
        LogError("GetLocalAddr:{}", strerror(errno));
        memset(&laddr, 0, sizeof laddr);
    }
...
std::string ToIPPort(const struct sockaddr_storage* ss)
{
    ...
    {
        LogError("unknown socket family connected");
        return empty_string;
    }
...
void SetTimeout(evpp_socket_t fd, uint32_t timeout_ms)
{
    ...
    {
        int err = errno;
        LogError("setsockopt SO_RCVTIMEO ERROR:{} {} ", err, strerror(err));
    }
}
...
void SetKeepAlive(evpp_socket_t fd, bool on)
{
    ...
    {
        int serrno = errno;
        LogError("setsockopt(SO_KEEPALIVE) failed, errno:{} {}", serrno, strerror(serrno));
    }
}
...
void SetReuseAddr(evpp_socket_t fd)
{
    ...
    {
        int serrno = errno;
        LogError("setsockopt(SO_REUSEADDR) failed, errno:{} {}", serrno, strerror(serrno));
    }
}
...
void SetReusePort(evpp_socket_t fd)
{
    ...
    int rc = ::setsockopt(fd, SOL_SOCKET, SO_REUSEPORT,
                          reinterpret_cast<const char*>(&optval), static_cast<socklen_t>(sizeof optval));
    LogInfo("setsockopt SO_REUSEPORT fd:{} rc:{}", fd, rc);
    if (rc != 0)
    {
        int serrno = errno;
        LogError("setsockopt(SO_REUSEPORT) failed, errno:{} {}", serrno, strerror(serrno));
    }
#endif
}
...
void SetTCPNoDelay(evpp_socket_t fd, bool on)
{
    ...
    {
        int serrno = errno;
        LogError("setsockopt(TCP_NODELAY) failed, errno:{} {}", serrno, strerror(serrno));
    }
}
