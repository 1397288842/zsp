...
{
    LogTrace("Start loop refcount:{}", self_.use_count());
    auto f = [this]()
    {
        ...
        timer_->Init();
        timer_->AsyncWait();
        LogTrace("Start loop refcount:{} periodic:{} timeout(ms):{}", self_.use_count(), periodic_, timeout_.Milliseconds());
    };
...
{
    LogTrace("OnTimerTriggered  use_count:{}", self_.use_count());
    functor_();
...
{
    LogTrace("OnCanceled use_count:{}", self_.use_count());
    periodic_ = false;
    if (cancel_callback_)
    {
        cancel_callback_();
