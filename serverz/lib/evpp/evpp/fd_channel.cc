...
namespace evpp
{
    ...
    {
        LogTrace("FdChannel::Close fd:{}", fd_);
        assert(event_);
        if (event_)
        {
            assert(!attached_);
    ...
    void FdChannel::AttachToLoop()
    {
        ...
        {
            LogInfo("fd:{}  watching event:{}", fd_, EventsToString());
            attached_ = true;
        }
        else
        {
            LogError(" fd:{} with event:{} attach to event loop failed", fd_, EventsToString());
        }
    }
    ...
    void FdChannel::DetachFromLoop()
    {
        ...
        {
            attached_ = false;
            LogInfo("fd:{} detach from event loop", fd_);
        }
        else
        {
            LogError("DetachFromLoop fd:{}  with event:{} detach from event loop failed", fd_, EventsToString());
        }
    }
