...
{
    LogTrace("remote_addr:{}", remote_addr());
    auto f = [this]()
    {
...
{
    LogInfo("TCPClient::DisconnectInLoop remote_addr:{}", remote_addr_);
    assert(loop_->IsInLoopThread());
    auto_reconnect_.store(false);
    ...
    if (conn_)
    {
        ...
        {
            LogError("already Disconnected/Disconnecting status:{}", conn_->StatusToString());
        }
...
void TCPClient::OnConnection(evpp_socket_t sockfd, const std::string& laddr)
{
    if (sockfd < 0)
    {
        LogError("Failed to connect to {}. errno:{} {}", remote_addr_, errno, strerror(errno));
    ...
    LogDebug("connected to:{} success", remote_addr_);
